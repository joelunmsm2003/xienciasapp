...
function Todo($scope,$http,$cookies,$filter) {
    ...
    $http.get("/querylote/"+$scope.database+"/"+lote).success(function(response) {
    ...
    $http.get("/enviarlote/"+lote).success(function(response) {
    ...
    $scope.Cancelarlote=function(currentPage){
         ...
         $http.get("/cancelar/"+lote).success(function(response) {
